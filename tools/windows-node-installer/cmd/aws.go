...
// newAWSCmd returns the AWS specific cobra command to be used for the windows installer tool
...
func createCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "creates a Windows instance under the same virtual network used by a given OpenShift " +
			"cluster running on the selected provider. " +
		...
		RunE: func(_ *cobra.Command, args []string) error {
			cloud, err := cloudprovider.CloudProviderFactory(rootInfo.kubeconfigPath, awsInfo.credentialPath,
				awsInfo.credentialAccountID, rootInfo.resourceTrackerDir, awsInfo.imageID, awsInfo.instanceType, awsInfo.sshKey)
			if err != nil {
